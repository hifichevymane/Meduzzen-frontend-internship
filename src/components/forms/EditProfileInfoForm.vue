  ...
  <form method="post" @submit.prevent="onSubmitUpdateUserData">
    ...
    <div v-if="isAbleToEdit && isUserRatingLoaded" class="d-flex mb-3 align-items-center gap-2">
      ...
      <div class="d-flex gap-2">
        <template v-if="!isUserRatingNotFound">
          <svg
        ...
        <template v-else>
          <span class="form-group-text">No rating data</span>
        </template>
      </div>
    </div>
...
const isUserRatingNotFound = ref(true)
...
onMounted(async () => {
  ...
  if (isAbleToEdit.value) {
    try {
      ...
      isUserRatingNotFound.value = false
    } catch (err) {
      isUserRatingLoaded.value = true
      isUserRatingNotFound.value = true
      store.commit('users/setErrorMessage', err.message)
    }
  }
})
